...
export function ReservationLookupContent({ isDashboard = false }: { isDashboard?: boolean }) {
  ...
  return (
    <div className={cn("min-h-screen bg-gray-50", { "min-h-0 bg-transparent": isDashboard })}>
      {isDashboard ? (
        <div className="mb-6">
          <h1 className="font-display text-2xl font-bold text-gray-900">Buscar Reserva</h1>
          <p className="text-gray-600 mt-1 mb-6">Ingresa el código de confirmación que recibiste al crear tu reserva.</p>
          <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-2 max-w-xl">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2.5 rounded-xl text-sm border border-gray-200 bg-white focus:outline-none focus:ring-2 focus:ring-orange-500 font-mono uppercase"
              />
            </div>
            <button type="submit" className="px-6 py-2.5 bg-orange-500 text-white font-semibold rounded-xl hover:bg-orange-600 transition-colors shadow-sm whitespace-nowrap">
              Buscar
            </button>
          </form>
        </div>
                  ...
                  {reservation.paymentStatus === 'PAYMENT_VERIFIED' ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="p-4 rounded-xl bg-orange-50 border border-orange-200 text-orange-900 text-sm">
                        <p className="font-semibold text-base mb-1">Adelanto requerido: S/ {reservation.advanceAmount.toFixed(2)}</p>
                        <p className="text-orange-700">Concepto del depósito: <code className="bg-orange-100 px-1.5 py-0.5 rounded font-mono text-xs">{reservation.confirmationCode}</code></p>
                      </div>

                      {/* Datos de pago del restaurante */}
                      {payCfg?.paymentInfo?.trim() || payCfg?.paymentQrUrl ? (
                        <div className="p-4 rounded-xl bg-white border-2 border-orange-300 shadow-sm space-y-3">
                          <h4 className="text-sm font-bold text-gray-900 flex items-center gap-2">
                            <Wallet className="h-4 w-4 text-orange-500" /> Datos de pago del restaurante
                          </h4>
                          {payCfg?.paymentInfo?.trim() && (
                            <div className="bg-gray-50 rounded-lg p-3 text-sm text-gray-800 whitespace-pre-line leading-relaxed border border-gray-100">
                              {payCfg.paymentInfo}
                            </div>
                          )}
                          {payCfg?.paymentQrUrl && (
                            <div className="flex flex-col items-center gap-2 pt-1">
                              ...
                              <img src={payCfg.paymentQrUrl} alt="QR de pago del restaurante" className="h-48 w-48 rounded-xl border-2 border-orange-200 object-contain bg-white shadow-sm" />
                              <p className="text-xs text-orange-600 font-medium">Escanea este QR con Yape o Plin para pagar</p>
                            </div>
                          )}
                        </div>
                      ) : (
                        <div className="p-3 rounded-xl bg-amber-50 border border-amber-200 text-amber-700 text-sm">
                          <p>El restaurante aún no registró sus datos de pago. Te llegarán en el correo de confirmación.</p>
                        </div>
                      )}
                      ...
                      {isAuthenticated ? (
                        <div className="p-4 rounded-xl bg-gray-50 border border-gray-200 space-y-3">
                          <p className="text-sm font-semibold text-gray-800">¿Ya realizaste el pago? Sube tu comprobante:</p>
                          <div className="flex flex-col sm:flex-row gap-2">
                            <select value={proofMethod} onChange={(e) => setProofMethod(e.target.value)} className="rounded-xl border border-gray-200 px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 bg-white">
                            ...
                            <button onClick={() => proofInputRef.current?.click()} disabled={uploadingProof} className="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-orange-500 hover:bg-orange-600 text-white text-sm font-semibold rounded-xl transition-colors disabled:opacity-60 shadow-sm">
                              {uploadingProof ? <Loader2 className="h-4 w-4 animate-spin" /> : <Upload className="h-4 w-4" />}
                              {uploadingProof ? 'Subiendo…' : 'Subir comprobante de pago'}
                            </button>
                          </div>
                          <p className="text-xs text-gray-400">Sube una captura o foto del comprobante de pago.</p>
                        </div>
                      ) : (
                        <Link href="/login" className="inline-flex items-center gap-1.5 text-sm font-semibold text-orange-600 hover:underline">
